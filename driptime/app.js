...
function render() {
  ...
  $('water').textContent = `${Math.round(water)}g`;
  $('water-total').textContent = `/ ${step.endWaterGrams}g`;
  $('pour-amount').textContent = amount > 0 ? `${language === 'ko' ? '이 단계에서' : 'This Step'} +${amount}g` : text('물 붓기 없음');
  ...
  $('total').textContent = language === 'ko' ? `전체 ${time(timer.elapsed)}${hasManual ? ' · 드로다운 직접 확인 포함' : ` / ${time(selected.targetSeconds)}`} · 누적 목표 ${Math.round(water)} / ${selected.waterGrams}g` : `Total ${time(timer.elapsed)}${hasManual ? ' · Manual Drawdown' : ` / ${time(selected.targetSeconds)}`} · Total Target ${Math.round(water)} / ${selected.waterGrams}g`;
