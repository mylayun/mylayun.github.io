...
function adjacent(id, step, empty) {
  ...
  if (step) { const detail = document.createElement('small'); detail.textContent = `${step.manualAdvance ? text('직접 확인') : time(step.endSeconds - step.startSeconds)} · ${language === 'ko' ? '누적' : 'Total'} ${step.endWaterGrams}ml`; el.append(detail); }
}
function renderRecipe() {
  ...
  $('metadata').textContent = `${text(selected.brewer)} · ${selected.coffeeGrams}g / ${selected.waterGrams}ml · ${selected.waterTemperature}°C · 1:${(selected.waterGrams / selected.coffeeGrams).toFixed(1)}`;
  $('grind').textContent = waterText(selected.grindNote);
  ...
  $('steps').replaceChildren(...selected.steps.map(s => {
    ...
    grams.textContent = `${s.endWaterGrams}ml`; body.append(name, detail); row.append(body, grams); return row;
  }));
  lastStatus = ''; render();
}
function render() {
  ...
  $('total').textContent = language === 'ko' ? `전체 ${time(timer.elapsed)}${hasManual ? ' · 드로다운 직접 확인 포함' : ` / ${time(selected.targetSeconds)}`} · 누적 목표 ${Math.round(water)} / ${selected.waterGrams}ml` : `Total ${time(timer.elapsed)}${hasManual ? ' · Manual Drawdown' : ` / ${time(selected.targetSeconds)}`} · Total Target ${Math.round(water)} / ${selected.waterGrams}ml`;
